					...
					if self.journal_id.type in ('sale','purchase'):
						if self.move_type in ('out_invoice', 'in_invoice','out_receipt','in_receipt'):
							if not self.journal_id.bi_sequence_id:
								...
								seq = self.journal_id.bi_sequence_id._next()
								vals['name'] = seq
								self.journal_id.sudo().write({'bi_sequence_next_number':self.env['ir.sequence'].search([('id', '=', self.journal_id.bi_sequence_id.id)]).number_next_actual})
							...
							if not self.journal_id.bi_refund_sequence_id:
								...
								seq = self.journal_id.bi_refund_sequence_id._next()
								vals['name'] = seq
								self.journal_id.sudo().write({'bi_refund_sequence_next_number':self.env['ir.sequence'].search([('id', '=', self.journal_id.bi_refund_sequence_id.id)]).number_next_actual})
